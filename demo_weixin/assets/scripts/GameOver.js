...
import LTextFieldType from '../plugin/lufylegend/text/LTextFieldType';
import LMouseEvent from '../plugin/lufylegend/events/LMouseEvent';
import { LButtonSample1 } from '../plugin/lufylegend/lib/ui/LButtonSample';
import { LMath } from '../plugin/lufylegend/lib/LString';
/**
 * @author lufy
 */
class GameOver extends LSprite {
    constructor() {
        super();
        this.init();
    }
    init() {
        ...
        let windowBitmap = new LBitmap(new LBitmapData(common.datalist['window']));
        self.overLayer.addChild(windowBitmap);
        let gameText = new LTextField();
        ...
        let distanceText = new LTextField();
        distanceText.text = 'DISTANCE : ' + common.gameBody.character.distanceObj.value;
        ...
        let inputLayer = new LSprite();
        let bitmap = new LBitmap(new LBitmapData(common.datalist['inputbox']));
        inputLayer.addChild(bitmap);
        let inputOne = new LTextField();
        inputOne.x = 130;
        inputOne.y = 125;
        ...
        let buttonUpdate = new LButtonSample1(' 成績更新 ');
        buttonUpdate.x = 70;
        buttonUpdate.y = 170;
        ...
        let buttonReplay = new LButtonSample1(' もう一度 ');
        buttonReplay.x = 220;
        buttonReplay.y = 170;
    ...
    getRank() {
        ...
        let str = '[{"index":0,"name":"test1","value01":"100","value02":"0","value03":"0"},{"index":1,"name":"test2","value01":"90","value02":"0","value03":"0"},{"index":2,"name":"test3","value01":"80","value02":"0","value03":"0"},{"index":3,"name":"test4","value01":"70","value02":"0","value03":"0"},{"index":4,"name":"test5","value01":"60","value02":"0","value03":"0"}] ';
        self.rankShow(str);
    }
    rankShow(data) {
        let self = this;
        data = JSON.parse(data);
        let labelText, bitmap;
        for (let i = 0;i < data.length;i++) {
            let obj = data[i];
            bitmap = new LBitmap(new LBitmapData(common.datalist['stage'], 32 * 18 + i * 32, 32 * 3, 32, 32));
            bitmap.x = 0;
